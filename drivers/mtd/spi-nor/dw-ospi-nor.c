		...
		, bool enable_quad)
{
	...
	if ((enable_quad == false) || (ops == SPI_NOR_OPS_WRITE)) {
		...
		case SNOR_PROTO_1_1_2:
			dev_dbg(nor->dev, "read dual\n");
			ctrl0 |= SPI_DUAL_FORMAT << SPI_SPI_FRF_OFFSET;
			break;
			...
			size_t len, size_t *retlen, const u_char *buf)
{
	...
	*retlen = 0;
	...
	ret = dw_spi_command_write(nor, opcode, 1 + nor->addr_width,
				   (u8 *)buf, len);
	if (ret == 0)
		*retlen = len;
}
		      ...
		      size_t len, size_t *retlen, u_char *buf)
{
	...
	*retlen = 0;
	while (to_read) {
		...
		ret = dw_spi_command_read(nor, nor->read_opcode, offset, ptr,
					  chunk);
		if (ret < 0)
			return ret;
