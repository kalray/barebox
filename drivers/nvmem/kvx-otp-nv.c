...
static int kvx_otp_nv_read(struct device_d *dev, const int offset,
			  void *_val, int bytes)
{
	struct kvx_otp_nv_priv *priv = dev->parent->priv;
	u8 *val = _val;
	u32 tmp, copy_size;
	u8 skip = offset & OTP_NV_ALIGN_MASK;
	int a_offset = offset & ~OTP_NV_ALIGN_MASK;

	while (bytes) {
		tmp = readl(priv->base + a_offset);
		if (skip != 0)
			copy_size = min(OTP_NV_ALIGN - skip, (int) bytes);
		else
			copy_size = min_t(int, bytes, sizeof(tmp));
