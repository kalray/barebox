...
static int kvx_read_serial(struct device_node *socinfo)
{
	...
	kvx_serial = basprintf("%sA-%d%c-%03d", lot_id, wafer_id, com_ap,
			       device_id);

	globalvar_add_simple_string("kvx.serial", &kvx_serial);
